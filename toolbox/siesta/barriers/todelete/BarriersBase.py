...
class SiestaBarriersBaseNick:
    ...
    def prepare_neb(self,image_name='image',fdf_name='input',overwrite=False):
        ...
        #if neb_path.suffix is not None:
        #    neb_path = neb_path.stem
    ...
    def plot_neb_results(self,neb_results_folder_name=''):
        """
        """
        from .utils_analysis import NEB_Barrier
        
        neb_results_path = self.neb_path / neb_results_folder_name
        print(neb_results_path)
        if neb_results_path.exists():
            print("YES")
            NEB_Barrier(neb_results_path,self.number_of_images,fig_name='NEB',dpi_in=600)
        else:
...
class ManualNEB(SiestaBarriersBaseNick):
    ...
    #def __init__(self,initial_structure,final_structure,number_of_images,interpolation_method ,path='image' ):
        ...
        super().__init__(neb_path = neb_path, path='image',images = [initial_structure])
        #super().__init__(path='image',images = [initial_structure])
