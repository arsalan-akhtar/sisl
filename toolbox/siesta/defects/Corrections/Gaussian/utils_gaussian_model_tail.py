...
def get_charge_model_sigma(limits,
    ...
    g = get_gaussian_3d(grid, defect_position, sigma,charge,volume,eta)
    # Get the offsets
    offsets = np.zeros(3)
    for axis in range(3):
    ...
    for dim0 in range(2):
        for dim1 in range(2):
            for dim2 in range(2):
                ...
                g = g + get_gaussian_3d(grid, 
                        defect_position + image_offset, 
                        sigma=sigma,
                        charge=charge,
                        volume=volume,
                        eta = eta,
...
def get_gaussian_3d(grid, position, sigma,charge,volume,eta):
    """
    ...
    #eta = 1.0 - w
    w = 1.0 - eta
    gaussian = charge * eta * N_beta * (np.exp(-(x**2 + y**2 + z**2) / (beta**2))) + ((1-eta) *charge / volume)
